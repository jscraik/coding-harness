...
- `review-feedback-eval-seed`: converts repeated late-stage remediation noise and raw review comments into deterministic eval-seed artifacts with current/stale/unmapped/unsafe classification, feedback provenance, bounded issue taxonomy, matched files, evidence refs, remediation source, failure class, and a concrete target surface.
...
- `agent-next-action-parity`: proves `harness next` recommendations remain discoverable from the agent-facing command catalog, keep stale or missing optional prompt context advisory, block clean handoff on non-ready `pr-closeout/v1` evidence, and reject false-ready closeout artifacts.
