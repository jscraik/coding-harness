...

{
	...
	if [[ -n "$base_ref" ]]; then
		git -C "$ROOT_DIR" diff --name-only --diff-filter=ACMR "$base_ref"...HEAD -- src scripts contracts docs/agents/linear-templates package.json tsconfig.json .diagramrc 2>/dev/null || true
	else
		git -C "$ROOT_DIR" diff --name-only --diff-filter=ACMR HEAD -- src scripts contracts docs/agents/linear-templates package.json tsconfig.json .diagramrc 2>/dev/null || true
	fi
} | awk '
