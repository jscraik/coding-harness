...
if [[ -f "$TOOLING_DOC_PATH" ]]; then
	required_tooling_doc_terms=("node" "pnpm" "python" "uv" "make" "rg" "fd" "jq" "prek" "diagram" "mise" "vale" "argos" "cosign" "cloudflared" "vitest" "ruff" "eslint" "agent-browser" "agentation" "ctx7" "mermaid-cli" "markdownlint-cli2" "wrangler" "beautiful-mermaid" "semgrep" "semver" "trivy" "rsearch" "wsearch")
	...
	if [[ -n "$installed_hooks_dir" && -d "$installed_hooks_dir" ]]; then
		for hook_name in pre-commit pre-push commit-msg; do
			...
			if [[ -f "$installed_hook" ]] && rg -q "# File generated by prek: https://github.com/j178/prek" "$installed_hook"; then
				if ! rg -F -q 'WORKTREE_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"' "$installed_hook" || ! rg -F -q 'PREK_HOME="${PREK_HOME:-$WORKTREE_ROOT/.cache/prek}"' "$installed_hook"; then
					echo "Error: installed prek hook '$hook_name' is missing worktree-local PREK_HOME patch"
