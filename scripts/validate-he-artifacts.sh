...
repo_abs=$(resolve_abs "$repo_root")
plan_abs=$(resolve_abs "$plan_path")
spec_abs=$(resolve_abs "$spec_path")

if ! path_inside_repo "$plan_abs"; then
  emit_json "blocked" "artifact_outside_repo" "plan path resolves outside repository root: $plan_path"
  exit 1
fi

if ! path_inside_repo "$spec_abs"; then
  emit_json "blocked" "artifact_outside_repo" "spec path resolves outside repository root: $spec_path"
  exit 1
fi
