...
const REQUIRED_DIAGRAM_TYPES = ["architecture", "dependency", "security", "auth"];
...
function resolveImport(fromFile, importPath) {
  ...
  for (const candidate of [jsStripped, jsStripped + ".ts", path.join(jsStripped, "index.ts")]) {
    if (fs.existsSync(candidate)) return candidate;
  }
  return null;
}
...
function fail(rule, severity, file, message) {
  ...
  const effectiveSeverity = baselineKeys.has(baselineKey) ? "baseline" : severity;
  violations.push({ rule, severity: effectiveSeverity, file: rel, message,
...
function checkCommandsNoCrossImport() {
  ...
  for (const file of commandFiles) {
    for (const imp of extractImports(file)) {
      const resolved = resolveImport(file, imp);
      if (resolved && resolved.startsWith(COMMANDS_DIR) && resolved !== file) {
        fail(
...
function checkDiagramFreshness() {
  ...
  try {
    ...
    fail("diagram-freshness", "error", MANIFEST_PATH, "Manifest JSON is invalid.");
    return;
  }
  ...
  for (const type of REQUIRED_DIAGRAM_TYPES) {
    ...
    const diagPath = path.join(ROOT, "AI", "diagrams", diag.path ?? `${type}.mmd`);
