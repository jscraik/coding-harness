...
- Treat runtime/source output as authoritative and use docs as supporting context.
- Use `harness next --json` as the fresh-agent entrypoint.
- Use focused `harness --help` for first-contact command truth only; use `harness commands --json` for the full machine-readable capability catalog and `harness commands --json --for-agent` for the public agent rail set.
- When maintaining the `coding-harness` source repository itself, prefer `pnpm exec tsx src/cli.ts` probes over a globally installed `harness` binary so command guidance matches the current source tree.
...
- Focused command surface: `harness --help`
- Full capability catalog: `harness commands --json`
...
- ❌ Reporting full capabilities from focused `harness --help` alone; use `harness commands --json` for full catalog truth.
