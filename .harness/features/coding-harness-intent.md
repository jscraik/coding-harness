...
- `src/commands/ci-migrate-core.ts` is a very large module. That is a module boundary failure even if the behavior is valuable.
- `src/commands/ci-migrate.test.ts` is a very large test file. That suggests the behavior is too entangled or the fixture strategy is too large.
- `src/commands/review-gate-core.ts` is a large cross-domain module. That is strategically important but fragile.
- `src/lib/contract/types-core.ts` is broad enough that readers need generated maps and targeted schemas.
- The command and TypeScript surface is broad enough that command discoverability is a product requirement, not a nice-to-have.
- `memory.json` appears to be bootstrap/sample memory rather than current operational memory, which conflicts with the repo's high standard for repo-local memory proof.
...
| --- | --- | --- | --- | --- | --- | --- |
| The repository is a control plane around agents, not an agent runtime | docs, config | `README.md:8-17`, `harness.contract.json:9-27` | North-star contract, product description | Durable proof is the checked-in identity contract; time-scoped preflight runs belong in review-log or closeout artifacts. | Hard evidence | The project identity is stated in both prose and contract data |
| PR lead time is the north-star metric | docs, config, runtime flow | `README.md:126-140`, `docs/roadmap/north-star.md:22-37`, `harness.contract.json:9-20` | `northStar.primaryMetric`, bottleneck `review_rework_loop` | Durable proof is the contract field and roadmap wording; current metric observations must be refreshed from live telemetry. | Hard evidence | It is the measure that should decide whether complexity is justified |
| The agent-native model is real | source-code | `src/commands/next.ts:120-260`, `src/lib/decision/harness-decision.ts:52-153`, `src/cli.ts:52-64` | `HarnessDecision`, `next`, robot mode | Durable proof is the implemented JSON decision surface; runtime status must be rechecked with the current CLI. | Hard evidence | The repo implements machine-readable agent decision support |
| Governance is intended to be executable | config, CI/CD, docs | `.harness/ci-required-checks.json:1-20`, `.circleci/config.yml:143-205`, `.github/PULL_REQUEST_TEMPLATE.md:11-21`, `docs/agents/04-validation.md:27-36` | Required checks, PR checklist, validation gates | Durable proof is the checked-in gate configuration and wrapper scripts; current pass/fail status belongs in validation evidence. | Hard evidence | Governance exists in CI and scripts, not just documentation |
...
| The contract schema is a central stable interface | source-code, config | `src/lib/contract/types-core.ts:1477-1538`, `src/lib/contract/types-core.ts:1540-1560`, `harness.contract.json` | `HarnessContract`, default contract | Durable proof is the typed contract schema plus checked-in contract instance; current validation must be rerun before release. | Hard evidence | Many governance behaviours depend on one contract spine |
| Command discoverability is a product requirement | source-code, naming patterns | `src/lib/cli/command-registry.ts:49-116`, `src/lib/cli/registry/command-capabilities.ts:3-47`, `src/lib/cli/registry/command-capabilities.ts:196-238` | Command registry, capability metadata | Source inventory found a broad command surface under `src/commands/` | Hard evidence | The command surface is too large for memory-only navigation |
| Validation is designed as structured evidence | source-code, scripts, docs | `src/commands/health-core.ts:31-79`, `src/commands/docs-gate-core.ts:83-125`, `scripts/verify-work.sh:43-60`, `docs/agents/04-validation.md:143-178` | Gate reports, verify-work, failure classes | Durable proof is the structured gate/reporting code; current gate outcomes belong in time-scoped validation records. | Hard evidence | The harness's value depends on repeatable proof, not terminal prose |
| Project Brain and Local Memory are intended operating surfaces | docs, runtime flow | `.harness/knowledge/INDEX.md:5-18`, `.harness/review-log.md:5-15`, `docs/agents/15-context-integrity-compact.md:59-84` | Project Brain domains, review log, context checkpoints | Durable proof is the tracked Project Brain map and review-log contract; live memory health must be refreshed per execution. | Hard evidence | Memory is treated as repo infrastructure, so drift here affects agent decisions |
| The repo is ahead of common agent-readiness standards | source-code, docs, config | `src/lib/decision/harness-decision.ts`, `src/lib/cli/registry/command-capabilities.ts`, `.github/PULL_REQUEST_TEMPLATE.md`, `docs/architecture/documentation-layers.md` | Decision envelope, command metadata, evidence PR template, docs layers | Durable proof is the source-defined decision and command metadata surfaces; runtime path health must be validated per PR. | Strong inference | Few repos encode this much agent-operational metadata as source |
...
| Large core modules are a maintainability risk | source-code, naming patterns | `src/commands/ci-migrate-core.ts`, `src/commands/ci-migrate.test.ts`, `src/commands/review-gate-core.ts`, `src/lib/contract/types-core.ts` | CI migration core, review gate core, contract types | Source inventory found very large core modules and test surfaces | Hard evidence | Module size increases regression risk and slows future extraction |
| Placeholder memory is a drift signal | config | `memory.json` | Baseline memory object | File appears to be bootstrap/sample memory rather than current operational memory | Hard evidence | A memory-first repo should not expose placeholder operational memory as truth |
...
| The smallest compelling version should center on the PR loop | docs, source-code, interpretation | `README.md:150-198`, `src/commands/next.ts`, `scripts/verify-work.sh`, `src/commands/review-gate-core.ts` | init, next, verify, review gate, required-check parity | Durable proof is the implemented PR-loop command surface; full PR-loop runtime evidence must be collected in slice-specific evals. | Strong inference | A narrow product loop would reduce adoption friction and preserve strategic focus |
