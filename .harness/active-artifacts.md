...
validation gate lane. Live GitHub showed zero open PRs, and PR #361 had
CodeRabbit passing, Socket passing or skipping only the non-blocking PR-alert
lane, all repo-owned CircleCI contexts passing, aggregate `pr-pipeline`
passing on pipeline `2524`, and aggregate `security-scan` passing on
pipeline `2524`. The external Snyk GitHub App quota/status failure remains an
owner-approved waiver for that external lane only; it does not claim external
Snyk passed and does not waive repo-run security, future Snyk results, or future
security findings. PR #360 merged at
