...
Thread PRRT_kwDORWZJCc6HbjKF was fixed locally by rejecting the
default-provider stale form `harness ci-migrate prepare --apply`. The latest
local blocker is Codex thread PRRT_kwDORWZJCc6HbpRQ, which found that the
validator still missed `--action prepare --apply`. The local repair now reads
both positional actions and `--action <value>` before applying the prepare plus
`--apply` guard. It must be pushed, reviewed, and swept. Push state, thread
resolution, aggregate CI, and merge readiness remain separate PR sweep truth.
