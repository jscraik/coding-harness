...
The latest local blocker is Codex thread PRRT_kwDORWZJCc6HbjKF, which found
that the validator still allowed the default-provider stale form
`harness ci-migrate prepare --apply`. The local repair rejects prepare plus
`--apply` regardless of whether `--provider circleci` is written explicitly.
It must be pushed, reviewed, and swept. Push state, thread resolution,
aggregate CI, and merge readiness remain separate PR sweep truth.
