...
Current working reconciliation: 2026-06-09 post-PR384 merge pullback plus
Linear blocker refresh for JSC-363. PR #384 is merged into `main` as squash
merge commit `96846c31b7d3b1bade77b1145543ab1c92c797ae`; local `main` and
`origin/main` are synced to that head. PR #383 remains merged validator-repair
...
route-truth comment `34a50024-24be-4853-af6e-3219cbc0d845`. The active branch
is `codex/jsc-363-post-pr384-linear-blocker-refresh`, which records that
PR #384 merged and that the Linear field-text decision remains blocked because the
repo Linear commands fail without `LINEAR_API_KEY`, while `~/.codex/.env` is a
FIFO rather than a safely sourceable regular env file.
...
- JSC-363 is the current execution route for the Codex Runtime Evidence
  ...
  commit `b7b5d9f48fb9369c29a4cd9902acde88285bfb22`. The current active PR lane
  is the post-PR384 Linear blocker refresh.
