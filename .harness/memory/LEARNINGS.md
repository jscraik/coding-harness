...
**2026-05-28 [Codex]:** Repeated slice-level judgment can stay trapped in conversation unless it has a promotion threshold -> promote the smallest durable primitive only when the current contract is contradictory, a safety boundary is crossed, or the same failure recurs across independent work; otherwise record `no_system_change` with reason, checked scope, and no-durable-destination decision, using implementation notes or plan evidence for one-off knowledge and skills only for reusable routed workflows with explicit inputs, artifacts, validation, ownership, and review expectations.

**2026-06-04 [Codex]:** Specific implementation-detail feedback was still at risk of being handled as a local fix despite pattern-generalization guidance -> classify specific feedback, review comments, line-level corrections, and function examples using the shared threshold; search sibling patterns and choose a validator, lint rule, schema constraint, shared utility, repository convention, CI check, documented invariant, or tracked exception only when a contradictory contract, crossed safety boundary, recurrence across independent work, or named current consumer requires it.
...
**2026-07-04 [Codex]:** Downstream canaries can fail when source-repo runtime fallback pins drift ahead of a consumer contract -> `harness check-environment` must prefer the consumer `harness.contract.json` `toolingPolicy.requiredMiseTools` pin and use the harness fallback only when the consumer omits that tool.

**2026-07-06 [Codex]:** Broad architectural lint feedback can be accidentally narrowed to a single-language canary even when the user or source note says harness-level guardrail family -> stop ordinary implementation, write a current-session steering admission, inventory repo languages/backends first, then build or update the smallest cross-language validator family with focused tests and `pnpm run docs:steering:guard` proof before resuming.
