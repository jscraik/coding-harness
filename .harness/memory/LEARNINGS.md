...
**2026-05-18 [Codex]:** `A heartbeat can become stale after its PR lane is already merged, and repeated DONT_NOTIFY replies still waste user tokens -> once the stop condition is true, delete or update the automation in the same turn; if the app automation API is unavailable, identify the exact repo-owned automation.toml, remove only that matching file, rmdir the empty directory, and verify the automation ID no longer appears before continuing.`
