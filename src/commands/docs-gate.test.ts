			...
			surfaces: [
				{
					...
					northStarContribution:
						"Prevents governance/docs drift on the lead-time path.",
					manualGlueReductionClaim:
						"Eliminates repeated manual documentation parity checks.",
					reliabilityContribution:
						"Produces deterministic docs drift findings with explicit remediation.",
					evidenceReference: "src/commands/docs-gate.ts",
					ownedPaths: ["src/commands/docs-gate.ts", "README.md"],
...
describe("docs-gate command", () => {
	...
	it("emits instruction_precedence_conflict when canonical docs disagree on discovery order", () => {
		const root = join(
			process.cwd(),
			"artifacts",
			"docs-gate-test-contradiction-4",
		);
		roots.push(root);
		createContractWithDocsGate(root, {
			enabled: true,
		...
		expect(
			result.report.findings.some(
				(finding) =>
					finding.category === "instruction_precedence_conflict" &&
					typeof finding.message === "string" &&
					finding.message.includes("first discovery source"),
			),
		).toBe(true);
	});

	it("detects discovery-order contradictions from fallback prose parsing", () => {
		const root = join(
			process.cwd(),
			"artifacts",
			"docs-gate-test-contradiction-4b",
		);
		roots.push(root);
		createContractWithDocsGate(root, {
			enabled: true,
		...
		expect(
			result.report.findings.some(
				(finding) => finding.category === "instruction_precedence_conflict",
			),
		).toBe(true);
	});

	it("emits workflow_policy_conflict when docs declare conflicting active providers", () => {
		const root = join(
			process.cwd(),
			"artifacts",
			"docs-gate-test-contradiction-5",
		);
		roots.push(root);
		createContractWithDocsGate(root, {
			enabled: true,
		...
		write(
			join(root, "README.md"),
			["# README", "", "Active provider: github-actions"].join("\n"),
		);
		write(
			join(root, "docs/agents/17-ci-required-checks.md"),
			["# CI Checks", "", "Active provider: circleci"].join("\n"),
		);
		...
		expect(
			result.report.findings.some(
				(finding) =>
					...
					typeof finding.message === "string" &&
					finding.message.includes("active providers"),
			),
		).toBe(true);
	});

	it("emits workflow_policy_conflict when docs active provider disagrees with contract", () => {
		const root = join(
			process.cwd(),
			"artifacts",
			"docs-gate-test-contradiction-5b",
		);
		roots.push(root);
		createContractWithDocsGate(root, {
			enabled: true,
		...
		expect(
			result.report.findings.some(
				(finding) =>
					finding.category === "workflow_policy_conflict" &&
					typeof finding.message === "string" &&
					finding.message.includes("contract requires 'circleci'"),
			),
		).toBe(true);
	});

	it("normalizes provider aliases and activeprovider syntax for workflow-policy checks", () => {
		const root = join(
			process.cwd(),
			"artifacts",
			"docs-gate-test-contradiction-provider-aliases",
		);
		roots.push(root);
		createContractWithDocsGate(root, {
			enabled: true,
		...
		write(
			join(root, "README.md"),
			["# README", "", "activeprovider = github actions"].join("\n"),
		);
	...
	it("resolves changed files from git diff when --files is omitted", () => {
		const root = join(
			process.cwd(),
			"artifacts",
			"docs-gate-test-git-diff-resolution",
		);
		roots.push(root);
		createContractWithDocsGate(root, {
			enabled: true,
		...
		write(
			...
			"export const command = 'docs-gate-test';\n",
		);
		...
		write(
			join(root, "src/cli.ts"),
			"export const command = 'docs-gate-test-updated';\n",
		);
		...
		expect(result.report.execution_context.changedFilesSource).toBe("git_diff");
		expect(result.report.changed_files).toContain("src/cli.ts");
		expect(result.report.outcome).toBe("drift_detected");
		expect(result.exitCode).toBe(10);
	});

	it("fails closed in required mode when changed-file git resolution fails", () => {
		const root = join(
			process.cwd(),
			"artifacts",
			"docs-gate-test-git-diff-failure-required",
		);
		roots.push(root);
		createContractWithDocsGate(root, {
			enabled: true,
		...
		try {
			...
			expect(result.report.execution_context.changedFilesSource).toBe(
				"full_repo_fallback",
			);
			expect(result.report.outcome).toBe("policy_error");
			expect(result.report.status).toBe("blocked");
			...
			expect(
				result.report.findings.some(
					(finding) =>
						finding.rule_id === "docs.gate.changed_files_resolution_error" &&
	...
	it("keeps advisory mode non-blocking when changed-file git resolution fails", () => {
		const root = join(
			process.cwd(),
			"artifacts",
			"docs-gate-test-git-diff-failure-advisory",
		);
		roots.push(root);
		createContractWithDocsGate(root, {
			enabled: true,
