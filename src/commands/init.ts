...
function renderScriptFallbackActionCommand(
	packageManager: string,
	scripts: string[],
): string {
  ...
  exit 0
	...
	const preferredScripts = scripts.join("/");

	return `set -euo pipefail

if [ ! -f package.json ]; then
  echo "[codex] package.json not found"
  exit 1
fi

${checks}

echo "[codex] no matching script found (${preferredScripts})"
exit 1`;
}

function renderCodexEnvironmentTemplate(packageManager: string): string {
	const installCommand = renderInstallCommand(packageManager);
	const runCommand = renderScriptFallbackActionCommand(packageManager, [
		"dev",
		"start",
	]);
	const debugCommand = renderScriptFallbackActionCommand(packageManager, [
		"check",
		"typecheck",
		"lint",
	]);
	const testCommand = renderScriptFallbackActionCommand(packageManager, [
		"test",
	]);
...
`,
	},
	{
		...
		render: (pm) => renderCodexEnvironmentTemplate(pm),
	},
	{
