...
const USAGE_ERROR_HANDLERS: Partial<Record<NextUsageError, UsageErrorHandler>> =
	{
		invalid_mode: (parsed) =>
			invalidModeDecision(parsed.errorValue ?? "unknown"),
		mode_missing: (parsed) =>
...
export function usageErrorDecision(
	...
	return parsed.error
		? USAGE_ERROR_HANDLERS[parsed.error]?.(parsed)
		: undefined;
}
