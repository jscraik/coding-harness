 ...
 *
 * The generated script verifies Codex environment action mappings, required
 * Makefile targets, and configured prek hooks (including name, entry,
 * language, pass_filenames, and optional stages). It also checks installed
 * git hooks for the worktree-local PREK_HOME patch (pre-commit, pre-push,
 * and commit-msg), validates required package.json scripts, and errors if
 * legacy simple-git-hooks configuration is present.
 *
