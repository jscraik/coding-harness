...
function renderHarnessCliPreamble(): string {
...
function renderHarnessCliSourceRepoBranch(
	packageManager: string,
	packageExecCommand: string,
	installCommand: string,
): string {
	return `if is_harness_source_repo; then
...
function renderHarnessCliInstalledPackageBranch(
	...
	return `CLI_PATH="$REPO_ROOT/node_modules/@brainwav/coding-harness/dist/cli.js"
...
export function renderHarnessCliWrapper(packageManager: string): string {
	...
	return `${renderHarnessCliPreamble()}

${renderHarnessCliDetectionHelpers()}

${renderHarnessCliNpmFallbackHelpers()}

${renderHarnessCliSourceRepoBranch(packageManager, packageExecCommand, installCommand)}

${renderHarnessCliInstalledPackageBranch(installCommand, addCommand, execCommand)}
...
export function renderHarnessGateRunner(packageManager: string): string {
	...
	const templatePath = fileURLToPath(
		new URL("../../../scripts/run-harness-gate.sh", import.meta.url),
	);
	return readFileSync(templatePath, "utf-8")
