...
export type { CIProvider, Template, TemplateRenderContext };
...
const COMMIT_MSG_HOOK_ENTRY = REQUIRED_PREK_HOOKS["commit-msg"].entry;
...
const BIOME_SCHEMA_VERSION = "2.4.12";
const BIOME_SCHEMA_URL = `https://biomejs.dev/schemas/${BIOME_SCHEMA_VERSION}/schema.json`;
const CIRCLECI_PRIMARY_GITHUB_CHECK = "pr-pipeline";
const CIRCLECI_BRANCH_PROTECTION_REQUIRED_CHECKS = [
	CIRCLECI_PRIMARY_GITHUB_CHECK,
	"security-scan",
	"CodeRabbit",
] as const;
...
function getBranchProtectionRequiredChecks(
	...
	if (ciProvider === "circleci") {
		return CIRCLECI_BRANCH_PROTECTION_REQUIRED_CHECKS;
	}
...
function getGithubCheckNameForManifestEntry(
	...
	if (NON_WORKFLOW_REQUIRED_CHECK_SET.has(displayName)) {
		return displayName;
	}
...
function renderRequiredChecksManifest(
	...
	const requiredChecks = getHarnessInternalRequiredChecks(context).map(
		(displayName, index) => ({
			...
			githubCheckName: getGithubCheckNameForManifestEntry(
				ciProvider,
				displayName,
			),
		}),
...
function renderCircleCIConfig(pm: string): string {
	...
	const lintCiCommand = renderScriptCommand(pm, "lint:ci");
	...
	const skillValidateCommand =
		pm === "npm" ? "npm run skill:validate" : `${pm} run skill:validate`;
	const checkCommand = renderScriptCommand(pm, "check");
	const linearSyncCommand =
		pm === "npm"
			? "npm exec tsx scripts/circleci-linear-sync.ts"
			: `${pm} exec tsx scripts/circleci-linear-sync.ts`;
	return `version: 2.1
...
jobs:
  pr-pipeline:
    ...
    steps:
      ...
      - run:
          name: Sync PR lifecycle to Linear
          when: always
          command: |
            if [[ -f scripts/circleci-linear-sync.ts ]]; then
              ${linearSyncCommand}
            else
              echo "Skipping Linear sync (scripts/circleci-linear-sync.ts not found)."
            fi
...
/**
 * Produces a bash helper that creates a dedicated worktree for one task.
 *
 * The generated script keeps task isolation project-local by creating one worktree per task, then either
 * creates a dedicated branch or keeps detached HEAD mode, and prints bootstrap commands for the new checkout.
 *
 * @returns A complete POSIX-compatible bash script as a string. When executed, the script exits with:
 *          - `0` on success,
 *          - `1` if the base ref is invalid or the branch/worktree path already exists,
...
function main() {
	...
	try {
		...
		console.info("  • commit-msg: ${COMMIT_MSG_HOOK_ENTRY}");
...
`,
	...
	{
		path: "scripts/check-commit-msg.sh",
...
MSG_FILE="$msg_file" make hooks-commit-msg
`,
	},
	{
...
if [[ -f "$TOOLING_DOC_PATH" ]]; then
	...
	required_default_install_hook_types=(${REQUIRED_PREK_INSTALL_HOOK_TYPES.map((hookType) => `"${hookType}"`).join(" ")})
	...
	if ! rg -q "^[[:space:]]*minimum_prek_version[[:space:]]*=[[:space:]]*\\"${REQUIRED_PREK_VERSION}\\"[[:space:]]*$" "$PREK_CONFIG_PATH"; then
		echo "Error: minimum_prek_version must be set to '${REQUIRED_PREK_VERSION}' in $PREK_CONFIG_PATH"
		exit 1
	fi

	required_prek_hooks=(${Object.entries(REQUIRED_PREK_HOOKS)
		.map(([hook, config]) => {
			const hookStages = "stages" in config ? config.stages.join(",") : "";
	...
	if [[ -d "$git_hooks_dir" ]]; then
		required_installed_hooks=(${REQUIRED_PREK_INSTALL_HOOK_TYPES.map((hookType) => `"${hookType}"`).join(" ")})
