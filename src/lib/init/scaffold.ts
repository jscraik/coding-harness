...
/**
 * Determine whether a scaffold template should be auto-updated for the given target path.
 *
 * Returns `true` only when `templatePath` is the Codex environment template path and the
 * target file is an autogenerated Codex environment (contains the generated-file header).
 *
 * @returns `true` if the template may be safely auto-updated, `false` otherwise.
 */
...
/**
 * Extracts the Biome schema version from a $schema URL.
 *
 * @param schemaUrl - The $schema URL to parse (for example: "https://biomejs.dev/schemas/2.4.14/schema.json")
 * @returns The version string (e.g. "2.4.14") if present, `undefined` otherwise
 */
