...
function getRepoRoot() {
	try {
		...
		throw new Error(\`Failed to determine repository root: \${error instanceof Error ? error.message : String(error)}\`);
	}
}

function getGitHooksDir() {
	try {
		const hooksPath = execFileSync("git", ["rev-parse", "--git-path", "hooks"], { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] }).trim();
		return resolve(process.cwd(), hooksPath);
	} catch (error) {
		throw new Error(\`Failed to determine git hooks directory: \${error instanceof Error ? error.message : String(error)}\`);
	}
}

function getPrekHome() {
...
function patchInstalledPrekHooks() {
	...
	let patchedCount = 0;
	for (const hookName of readdirSync(gitHooksDir)) {
		const hookPath = resolve(gitHooksDir, hookName);
		let hookContent = "";
		try {
...
function renderSetupGitHooksScriptMain(): string {
	return `
function main() {
	...
	try {
		...
		execFileSync("bash", ["scripts/run-prek.sh", "install", "--overwrite"], { env: { ...process.env, PREK_HOME }, stdio: "inherit" });
...
export function renderSetupGitHooksScript(): string {
	return `${renderSetupGitHooksScriptPrelude()}
${renderSetupGitHooksScriptMain()}`;
}
