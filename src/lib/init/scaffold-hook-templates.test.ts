...
describe("git-hook scaffold templates", () => {
	...
	it("renders the prek hook installer with worktree-local cache patching", () => {
		...
		expect(script).toContain("function getRepoRoot()");
		expect(script).toContain(
	...
	it("patches an inherited prek cache assignment in an installed hook", () => {
		const root = mkdtempSync(join(tmpdir(), "scaffold-hook-installer-"));
		const scriptsDir = join(root, "scripts");
		const binDir = join(root, "bin");
		mkdirSync(scriptsDir, { recursive: true });
		mkdirSync(binDir, { recursive: true });
		writeFileSync(join(root, "prek.toml"), "repos: []\n");
		writeFileSync(join(scriptsDir, "validate-commit-msg.js"), "");
		writeFileSync(
			join(scriptsDir, "setup-git-hooks.js"),
			renderSetupGitHooksScript(),
		);
		writeFileSync(
			join(scriptsDir, "run-prek.sh"),
			[
				"#!/usr/bin/env bash",
				"set -euo pipefail",
				"mkdir -p .git/hooks",
				...
				"  'exec \"$PREK\" hook-impl' > .git/hooks/pre-commit",
				"",
			].join("\n"),
		);
		writeFileSync(
			...
			[
				...
				"  pwd",
				"elif [[ $1 == rev-parse && $2 == --git-path && $3 == hooks ]]; then",
				"  printf '.git/hooks\\n'",
				"fi",
				"",
			].join("\n"),
		);
		chmodSync(join(scriptsDir, "run-prek.sh"), 0o755);
		...
		try {
			...
			expect(hook).toContain('PREK_HOME="$WORKTREE_ROOT/.cache/prek"');
