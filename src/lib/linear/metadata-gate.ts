...
function buildGateFailComment(
	...
	if (bounceTarget === "triage") {
		lines.push("This issue is being bounced back to **Triage** until the missing fields are resolved.");
	} else {
		lines.push("This issue can remain in **Todo** but cannot enter **In Progress** until acceptance criteria are added.");
	}
	lines.push("");
	lines.push("_This comment was generated by the metadata gate enforcement (JSC-193)._");
