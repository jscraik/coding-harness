...
const BLOCKER_FIELD_PATTERNS: Record<RequiredBlockerField, RegExp[]> = {
	...
	owner: [
		/\b(?:blocker?\s+owner|dependency\s+owner|owner)\s*[:=]\s*.+/i,
	],
	unblockCondition: [
	...
	for (const field of REQUIRED_BLOCKER_FIELDS) {
		...
		if (!hasExplicitField && !hasDescriptionPattern) {
			...
			const fieldDescriptions: Record<RequiredBlockerField, string> = {
				...
				owner:
					"Must specify who owns the blocking dependency.",
				unblockCondition:
...
export function generateBlockedReport(options: {
	...
	for (const issue of options.issues) {
		...
		if (!metadataValidation.valid) {
			...
			recommendedAction = "Nudge dependency owner; consider parallel workaround.";
		} else if (severity === "warning") {
			recommendedAction = "Monitor; reach out to dependency owner if no update.";
		} else {
...
export function buildBlockedEscalationComment(options: {
	...
	lines.push("");
	lines.push("_This alert was generated by the blocked governance model (JSC-196)._");
