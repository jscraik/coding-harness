...
function isValidNorthStarContract(value: unknown): value is NorthStarContract {
	if (!isPlainObject(value)) {
		return false;
	}
	const record = value as Record<string, unknown>;
...
function isValidProductSurfaceRegistry(
	value: unknown,
...
function isValidOverrideReviewerRegistry(
	value: unknown,
