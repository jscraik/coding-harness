...
export const REPEATED_ERROR_RESEARCH_SIGNAL_PATTERN =
	/(?:(?:(?:same error (?:happened|occurred)?\s*twice|same failure twice|same command failed twice|failed again with the same (?:error|failure|command|stack trace|exception)|same (?:stack trace|exception) (?:appeared|happened|occurred|recurred|repeated).*twice|same error repeated|don\u2019t fight (?:the )?(same )?error|don't fight (?:the )?(same )?error)[\s\S]{0,240}(?:across independent (?:tasks|work)|(?:current|existing) contract is contradictory|contract is contradictory|contradictory contract|(?:crossed|requires|implicates|violates) (?:a )?safety boundary|safety boundary is (?:crossed|required|implicated|violated))|(?:across independent (?:tasks|work)|(?:current|existing) contract is contradictory|contract is contradictory|contradictory contract|(?:crossed|requires|implicates|violates) (?:a )?safety boundary|safety boundary is (?:crossed|required|implicated|violated))[\s\S]{0,240}(?:same error (?:happened|occurred)?\s*twice|same failure twice|same command failed twice|failed again with the same (?:error|failure|command|stack trace|exception)|same (?:stack trace|exception) (?:appeared|happened|occurred|recurred|repeated).*twice|same error repeated|don\u2019t fight (?:the )?(same )?error|don't fight (?:the )?(same )?error))|(?:failure|error|command|test) recurs? across independent (?:tasks|work)|recurrence across independent (?:tasks|work))/i;
export const PATTERN_SCOPE_SIGNAL_PATTERN =
	/(?:(?:line[- ]level correction)[\s\S]{0,500}(?:current|existing) contract is contradictory|(?:pattern-generalization|shared pattern|sibling implementations|sibling pattern|shared abstraction)[\s\S]{0,500}(?:threshold|principle|search|inventory|changed|unchanged|local|systemic|generaliz|contradictory|(?:crossed|requires|implicates|violates) (?:a )?safety boundary|safety boundary is (?:crossed|required|implicated|violated)|recurrence across independent (?:tasks|work)|failure recurs across independent (?:tasks|work)|named current consumer)|(?:threshold|contradictory (?:current |shared )?contract|(?:current|existing) contract is contradictory|recurrence across independent (?:tasks|work)|failure recurs across independent (?:tasks|work)|(?:crossed|requires|implicates|violates) (?:a )?safety boundary|safety boundary is (?:crossed|required|implicated|violated)|named current consumer)[\s\S]{0,500}(?:pattern|sibling|shared abstraction|design|consumer))/i;
