...
| Pattern-Generalization Pass | The required follow-up after line-level or example-based feedback: specific implementation-detail feedback is systemic until proven isolated, so name the implied design/API principle, classify the correction as local or systemic, search sibling implementations and similar classes of misbehavior, update the shared pattern when appropriate, or record why the correction is intentionally local. Decide whether the rule belongs in a validator, lint rule, schema constraint, shared utility, repository convention, CI check, documented invariant, or tracked exception. |
...
| "Do the preflight" | Execute bootstrap gate | "Run `bash scripts/codex-preflight.sh --stack auto --mode optional`; report Local Memory failures as diagnostics. Use `--mode required` only for an explicit diagnostic or acceptance lane." |
