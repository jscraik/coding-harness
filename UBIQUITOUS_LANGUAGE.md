...
| Tool Promotion Threshold | The rule for deciding whether repeated judgment becomes a repo primitive: if the same judgment is needed twice, or the failure mode can recur across slices, promote it into the smallest durable primitive that changes future behavior. One-off implementation knowledge stays in implementation notes, plan evidence, or PR closeout evidence; deterministic rules become validators or guards; repeatable operator actions become CLI helpers; reusable routed workflows become skills only when they have explicit inputs, artifacts, validation, ownership, and review expectations. |
...
| CircleCI Env-Backed API Triage | The CircleCI API, CircleCI log, or CircleCI job recovery path: load `~/.codex/.env`, resolve `CIRCLECI_TOKEN`, `CIRCLE_TOKEN`, or `CIRCLE_API_TOKEN` without printing values, pass the token as `Circle-Token`, and use bounded network calls such as `curl --max-time` before calling CircleCI evidence unavailable. |
| Safe PR Body File Handoff | The PR body workflow for Markdown, backticks, command snippets, or generated validation output: write a non-interpreting body file, validate it with `pr-template-gate --pr-body-file`, and pass it with `--body-file` instead of shell interpolation, command substitution, or a raw `--body` string. |
