...
| Tool Promotion Threshold | The decision rule for turning a correction into infrastructure: promote it into the smallest durable primitive only when the current contract is contradictory, a safety boundary is crossed, or the same failure recurs across independent work. Keep one-off implementation knowledge and isolated repeated failures in implementation notes, plan evidence, or PR closeout evidence. Create or update a skill only for a reusable routed workflow with explicit inputs, artifacts, validation, ownership, and review expectations. |
