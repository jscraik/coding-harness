...
| Pattern-Generalization Pass | The required follow-up after line-level feedback: name the implied design principle, search sibling implementations, update the shared pattern when appropriate, or record why the correction is intentionally local. |
| Pattern Scope Inventory | Closeout evidence for pattern-generalization work: the inferred principle, sibling implementations searched, siblings changed, siblings intentionally left unchanged with reasons, and deferred follow-ups with tracker or evidence. |
...
| "do not just fix that line" | Pattern scope inventory | "Infer the API/design principle, search sibling implementations, update the shared pattern or all matching siblings, and record unchanged or deferred siblings with reasons." |
