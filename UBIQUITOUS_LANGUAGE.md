...
| Repeated-Error Research Pass | The required escalation when the same command, test, or runtime error recurs across independent tasks, the current contract is contradictory, or a safety boundary is implicated: stop local retries, research trusted web or upstream sources, list 3-5 candidate fixes, choose the most efficient repo-fit fix, implement it, and record the evidence in PR closeout. A one-off bounded recovery records `no_system_change` with its reason, checked scope, and no-durable-destination decision. |
| Env-Backed Validation Recovery | The required recovery step before calling a credentialed validation lane blocked: check `~/.codex/.env` for required variable names without printing values, use `op run --env-file ~/.codex/.env -- <command>` or a FIFO-aware loader when that path is a FIFO, source it only when it is a regular readable file, and only then classify missing or unavailable credentials. |
| Tool Promotion Threshold | The decision rule for turning a correction into infrastructure: promote it into the smallest durable primitive only when the current contract is contradictory, a safety boundary is crossed, or the same failure recurs across independent work. Keep one-off implementation knowledge and isolated repeated failures in implementation notes, plan evidence, or PR closeout evidence; when no durable destination is justified, record `no_system_change` with the reason, checked scope, and no-durable-destination decision. Create or update a skill only for a reusable routed workflow with explicit inputs, artifacts, validation, ownership, and review expectations. |
| CircleCI Env-Backed API Triage | The required recovery path before calling a CircleCI API, CircleCI log, or CircleCI job lane unavailable: load `~/.codex/.env` with `op run --env-file ~/.codex/.env -- <command>` when that path is a FIFO, source it only when it is a regular readable file, resolve `CIRCLECI_TOKEN`, `CIRCLE_TOKEN`, or `CIRCLE_API_TOKEN` without printing values, and use bounded network calls such as `curl --max-time` before classifying the API lane as blocked. |
...
| Pattern-Generalization Pass | The bounded follow-up when the shared-control threshold is met or a named current consumer requires it: name the implied design/API principle, classify the correction as local or systemic, search sibling implementations and similar classes of misbehavior, update the shared pattern when appropriate, or record `no_system_change` with its reason, checked scope, and no-durable-destination decision. Decide whether the rule belongs in a validator, lint rule, schema constraint, shared utility, repository convention, CI check, documented invariant, or tracked exception. |
...
| "Do the preflight" | Execute bootstrap gate | "Run `bash scripts/codex-preflight.sh --stack auto --mode optional`; routine mode does not invoke Local Memory. Use `--mode required` only for an explicit diagnostic or acceptance lane." |
...
| "don't fight errors" | Repeated-error research pass | "When the same error recurs across independent tasks, the current contract is contradictory, or a safety boundary is implicated, stop retrying locally, research web/upstream sources, list 3-5 possible fixes, choose the most efficient repo-fit fix, implement it, and record the research evidence. For one-off recovery, record `no_system_change` with reason, checked scope, and no durable destination." |
...
| "do not just fix that line" | Pattern scope inventory | "Infer the API/design principle and run the pattern-generalization pass only when the shared threshold or a named current consumer requires it; otherwise record local `no_system_change` with reason, checked scope, and no-durable-destination decision." |
| "this was an example" | Principle signal | "Treat the concrete correction as a signal to classify, not an automatic expansion rule; run the pattern-generalization pass only when the shared threshold or a named current consumer applies, otherwise record local `no_system_change` with reason, checked scope, and no-durable-destination decision." |
