...
- Diagram refresh validation must use the repo-owned `pnpm exec diagram` path for availability checks and generation; do not require a globally installed `diagram` binary when the package-manager scoped command is available.
