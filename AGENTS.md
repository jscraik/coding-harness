...
- SynAIpse context failures use an additive versioned envelope: optional observations may remain `contextUnknowns`, while lifecycle/access/stale/catalog/required-transition failures remain blocking; compatibility-map producers, parsers, decision and legacy readers, and runtime packets so `harness next --json` cannot downgrade blockers.
