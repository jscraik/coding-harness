...
- Git hooks must be installed through `make hooks`, `make setup`, or `node scripts/setup-git-hooks.js`; `scripts/check-environment.sh` fails generated `prek` `pre-commit`, `pre-push`, or `commit-msg` shims that do not export worktree-local `PREK_HOME="${PREK_HOME:-$WORKTREE_ROOT/.cache/prek}"` after deriving `WORKTREE_ROOT` from `git rev-parse --show-toplevel`.
