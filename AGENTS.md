...
- PR-template release-boundary, Durable evidence map, changelog, regression-plan,
  or AI traceability changes must keep the source template, scaffolded
  downstream template, validator rules, CHANGELOG entry, and review handoff
  docs synchronized without pasting raw transcripts, prompts, secrets, or
  bulky telemetry.
...
- Tooling-audit governance: validate effective `prek` stages and leaf commands through syntax-aware TOML parsing. Approved readiness forwarding wrappers must match the exact strict-mode, canonical-script-directory, single-`exec` shape documented in the tooling policy; comments may be valid syntax, but extra commands, traversal, recursion, and injection must fail closed.
- Review-learning closeout is advisory evidence only: `review-learning-closeout/v1` may record exact or fuzzy matches, promoted guardrails, and explicit skipped promotions, while missing learning or enforcement evidence remains `n.a.` with a concrete reason and never proves validation, review, CI, acceptance, release, or merge readiness.
