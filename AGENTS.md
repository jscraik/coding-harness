...
- Flow Ops closure-evidence merge repairs that touch generated environment setup, init scaffolding tests, or architecture context must keep the docs-gate-required README, security/governance, and architecture-bootstrap surfaces in the branch before push.
- When tooling/runtime changes touch generated-environment, validation/required-check, check-SHA behavior, or architecture-context categories, land the same PR with synchronized updates to README.md, AGENTS.md, CONTRIBUTING.md, and docs/agents/00-architecture-bootstrap.md as required closeout evidence (not optional follow-up notes).
