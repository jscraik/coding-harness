...
`9919a1b1eb35365caed7298a776c6b27922464ed`. Live GitHub shows PR #365 is
open and mergeable; repo-owned CircleCI lanes, aggregate `pr-pipeline`,
aggregate `security-scan`, CodeRabbit, Socket, and CircleCI
`snyk-dependency-scan` are green. The remaining failed check is the
owner-waived external Snyk GitHub App quota/status lane. PR #365 also had one
unresolved CodeRabbit receipt-boundary thread; this repair updates the affected
receipt anchors and must be refreshed before merge. This section records the
current route tracker after the PR #364 one-PR sweep; older PR-specific receipt
...
- PR #365 is the active route-tracker repair PR for PR #364 evidence. Its current head is `9919a1b1eb35365caed7298a776c6b27922464ed`; live repo-owned PR-template and aggregate PR checks are green, while the external Snyk GitHub App quota/status lane remains owner-waived only. It must clear the receipt-boundary review thread, merge, and be pulled back to local main before PU-013 implementation starts. PR #364 remains the current main evidence for selecting PU-013; PR #363 remains historical route evidence, not the current main head.
...
- Current route PR: PR #365 is open as the route-tracker repair lane after PR #364. Live repo-owned checks are green on head `9919a1b1eb35365caed7298a776c6b27922464ed`; the external Snyk GitHub App quota/status lane is owner-waived only, and review-thread refresh must confirm the receipt-boundary repair before merge.
