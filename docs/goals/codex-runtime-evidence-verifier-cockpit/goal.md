...
- PR #350 is open for the focused repair at pushed head `ba3a021ff7b7fb285e1a252046564dde06e622d8`. CodeRabbit, Socket, repo-owned security checks, the PR body gate, and visible CircleCI job contexts pass on that head at last refresh, while aggregate CircleCI `pr-pipeline` and `ci/circleci: orb-pinning` were still pending. GitHub still blocks merge because fresh review threads flagged route-truth and receipt-freshness contradictions in the goal control-plane surfaces.
