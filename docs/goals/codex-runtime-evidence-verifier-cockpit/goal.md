...
- PR #350 is open for the focused repair at pushed head `b251139d6c515b28aed7ebde81f93e028f958366`, with local follow-up commit `ee903d486c3c04961a061f21ccd226af832c20c5` pending push for the structured-surface staged-context validator finding. CodeRabbit, Socket, repo-owned security checks, the PR body gate, aggregate CircleCI, and visible CircleCI job contexts passed before that local follow-up, except the owner-waived external Snyk GitHub App quota lane. GitHub still blocks merge because Codex thread `PRRT_kwDORWZJCc6HbVQu` found the validator treated structured JSON/YAML surfaces as whole-file staged migration context and could reject a valid standalone one-shot apply field beside staged snapshot fields.
