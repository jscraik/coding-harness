...
| Prompt/context authority classification | PU-051/CNF-005 commit `861c3e3b6a37ed2b801ab9ff27f6b31b68d8ef12` adds required `authorityLayer` metadata to `prompt-context-receipt/v1` source refs in `src/lib/prompt-context/`, keeps `instructionSources` limited to instruction authority layers and instruction-capable source kinds, and documents the non-claim boundary in architecture/governance docs. PR #335 received a review fix that now rejects `plugin`, `mcp`, `goal`, telemetry, artifact data, review feedback, and untrusted external refs when presented as `instructionSources`. | Preserve explicit authority and source-kind classification before a source may steer agent behavior. Keep prompt-context receipts pointer-only, `not_yet_emitted`, and unable to authorize commands, satisfy delivery-truth claims, prove PR/CI truth, or close Judge/PM readiness. | Implemented locally in `src/lib/prompt-context/prompt-context-receipt.ts`, `contracts/prompt-context-receipt.schema.json`, the checked-in example, focused receipt/schema tests, architecture docs, governance docs, intent artifact, and review evidence. Missing, unknown, orientation-only, and non-instruction-capable instruction sources are rejected. PR #335 has been pushed to head `8b70a32d909c0c72db47305d5ba686bc0e073479` and requires live checks, CodeRabbit, review-thread, Snyk, mergeability, delivery-truth, Judge/PM, and parent-goal refresh before any completion claim. |
| Steering queue application receipts | PU-052/CNF-006 local slice adds `steering-application-receipt/v1` in `src/lib/steering-queue/` with a pointer-only, `not_yet_emitted` contract for queue item, expected/current thread-turn-message context, stale-precondition result, runtime-card update ref, blockers, and head-SHA freshness. | Preserve application receipts as advisory orientation/audit evidence only; they must not authorize commands, mutate runtime cards, prove delivery truth, prove Judge/PM or merge readiness, mutate trackers, or close the parent goal until a future producer/consumer boundary is implemented and validated. | Implemented and pushed on `codex/jsc-363-cnf-006-steering-application-receipt`; draft PR #336 is being reconciled against the updated PR #335 head `8b70a32d909c0c72db47305d5ba686bc0e073479`. Positive and negative fixtures cover expected/current mismatch, expired steering, superseded items, missing runtime-card ref, applied receipt head mismatch, blocked expired/superseded receipts, and raw prompt-like key rejection. Required independent reviewer artifacts remain blocked by runtime artifact-output failure, and remote checks/reviews require refresh after this merge repair. No remote-green, merge-ready, delivery-truth, Judge/PM, or parent-goal completion claim is made. |
