...
artifacts/agent-runs/<runId>/events.jsonl` when the run should also append a
canonical replay-ready `agent-run-event/v1` JSONL stream and sibling run
manifest. Trace output is opt-in audit/orientation evidence only; it does not
prove closeout, review coverage, CI, Linear alignment, or merge readiness.
