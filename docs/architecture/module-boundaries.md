...
`fleet-plan-command-spec.ts`, next delegation in `next-command-spec.ts`, PR
template gate option projection in
...
- `src/lib/cli/registry/command-specs-core.ts` must remain a manifest assembler
  (`<= 1588` lines); workflow-specific parsing belongs in focused command spec
  seams.
