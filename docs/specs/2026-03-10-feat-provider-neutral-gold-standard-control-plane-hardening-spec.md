...
Logical run-level record used for governance and evaluation. It may be represented as canonical manifest extensions, companion records, or both, but consumers must observe one unified model.
...
6. Run `docs-gate` or consume its trusted machine-readable artifact for governed instruction/doc surfaces.
7. Produce an `InstructionParityResult` by translating the `docs-gate` result into control-plane terminology without changing its blocker meaning.
...
- Companion artifacts must live alongside the canonical run artifacts in the same run directory and use the same `runId`.
...
12. Overrides by a human or automation must be recorded with provenance.
13. Sensitive fields must be sanitized before persistence or telemetry export.
...
- capture any operator override with reason and provenance;
...
1. Should the provider-neutral layer extend the canonical run manifest/event schemas directly, or should it use companion control-plane artifacts that reference the existing schemas?
2. Which exact instruction-surface set should be scaffolded by default in `init` for non-Codex clients:
   - `CLAUDE.md` only,
   - `CLAUDE.md` plus `GEMINI.md`,
   - or a registry-driven generated set?
3. Should instruction-parity enforcement land through an extended `docs-gate`, a new dedicated control-plane gate, or a coordinated pair of checks with explicit ownership boundaries?
   Current state: `docs-gate` already owns document-surface coverage and machine-readable reporting, but contradiction counting is still placeholder-level and should not be overclaimed.
4. How should the current `branch-enforcer` migrate from `codex/*` semantics to a provider-neutral rule model without losing existing safety value?
5. Which scorecard thresholds are merge-authoritative in the harness repo versus advisory-only in downstream initialized repositories?
