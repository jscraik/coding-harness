...
For source-checkout proof, run `pnpm install --frozen-lockfile` first. Use
`node --import tsx src/cli.ts` before a build, or run `pnpm build` and then use
`pnpm exec harness`; a bare global `harness` binary is installed-consumer
evidence only.
...
- per-release product-surface cadence repairs are owned by the `owner` declared
  ...
  `AGENTS.md`, and this guide synchronized. Rerun
  `bash scripts/run-harness-gate.sh drift-gate --mode health --json`: exit `0`
  means clear, status `blocked` with exit `1` means out of cadence, and exit `2`
  means execution error.
...
- preflight or Local Memory enforcement changes should keep `AGENTS.md`, `README.md`, `docs/agents/02-tooling-policy.md`, `docs/agents/03-local-memory.md`, `docs/agents/06-security-and-governance.md`, and this guide synchronized; routine and legacy positional `scripts/codex-preflight.sh` invocations do not invoke Local Memory, while explicit `--mode required` remains fail-closed. Runtime/template mirrors must stay aligned, and test-only runner isolation must fail closed whenever CI is present
...
- SynAIpse context-plane changes should keep `synaipse-context-catalog/v1`,
  ...
  cross hosted or public boundaries. The additive versioned failure envelope is
  the canonical producer/parser/decision/legacy-reader/runtime boundary:
  optional provider, missing, or unresolved-host observations may project as
  `contextUnknowns`, while lifecycle, access, stale-digest, catalog, and
  required-transition failures remain blocking and retain their recovery and
  stop semantics. Keep the compatibility matrix and semantic fixtures aligned
