...
- stale-document archive-candidate reporting changes that add or alter `pnpm docs:archive-candidates`, archive-candidate schemas, docs-gate projection, or stale-document repair hints should keep `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, tooling policy, security governance, and this guide synchronized; the report is advisory-only and cannot archive, move, delete, demote, or rewrite documentation without a separate reviewed decision
