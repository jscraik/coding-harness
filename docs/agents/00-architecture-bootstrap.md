...
Agent-only commands are exposed on the dedicated
`harness commands --json --for-agent` rail; an explicit `--all` or
`--plumbing` request may include them with the complete internal catalogue.
...
Dependency-audit transport is part of this graph. Keep the aggregate command,
validation graph, validation-plan output, and generated scaffold command aligned
on `pnpm run audit` so the package-manager built-in cannot bypass the
repository-owned bulk advisory and public-scope policy boundary.
...
just prose hygiene. When the current contract is contradictory, a safety
boundary is crossed, the same failure recurs across independent work, or a
named current consumer requires the rule, encode it as the smallest durable
validator, tool, or skill hook and keep the generated architecture artifacts
synchronized. If the knowledge is genuinely one-off implementation context,
keep it in plan evidence or implementation notes instead of adding a new
execution surface.
