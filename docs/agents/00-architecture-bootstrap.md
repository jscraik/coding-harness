...
The refresh script must use the repo-owned `pnpm exec diagram` path for both
availability checks and generation. Do not require a globally installed
`diagram` binary when the package-manager scoped command is available,
