...
- **Behavior**: Classifies changed files into impact categories; verifies required docs exist, including tracked workflow-authority docs such as `docs/agents/01-instruction-map.md`, `docs/agents/04-validation.md`, `docs/agents/08-release-and-change-control.md`, `docs/agents/10-agent-testing-gates.md`, `docs/agents/13-linear-production-workflow.md`, `docs/agents/14-docs-gate-rollout.md`, `docs/agents/15-context-integrity-compact.md`, and `docs/agents/16-linear-production-compact.md`, plus tracked compound-workflow artifacts under `docs/adr/`, `docs/specs/`, `docs/plans/`, and `docs/brainstorms/`. It also enforces documentation lifecycle metadata and downstream-template distribution boundaries for governed docs, plus the promoted CodeRabbit frontmatter learning: policy docs with YAML frontmatter fields such as `schema_version`, `status`, or `applies_to` must keep those fields as metadata, not body headings or Table of Contents entries.
...
- Run `pnpm run quality:behavior-tests` when evidence-bearing trust-boundary suites or their manifest change.
- Run `pnpm run quality:git-env-sanitizer` when git child-process environment handling changes.
- Run `pnpm run harness:audit-tracking` when `.harness/audits/`, `.gitignore`, or the `.harness` control-plane map changes.
...
- For bounded live-canary planning, use `pnpm test:harness-canary -- --json <repo>...`. The read-only `harness-canary-audit/v1` report combines `next`, the existing upgrade matrix, and optional `fitness --from-existing-artifacts` evidence, and proves each target's HEAD and status are unchanged. Missing fitness artifacts are warnings, not readiness proof.
...
- For this repository, keep `## Release boundary` structured with `Release mode`, `Completion condition`, `Deferred work`, and `Stronger-proof condition`. These fields keep Prototype, Portfolio, Product, and Harness changes within their selected release standard.
- Keep the always-required `## Change details` fields focused on scope, traceability, documentation, SemVer, runtime impact, and acceptance evidence. Complete `Validation evidence`, `Review artifacts`, `Pattern scope inventory`, `Meta-behavior proof`, `Repeated-error research`, `Durable evidence map`, and `Learning / reinforcement` only when their admission conditions apply.
...
- For stacked PRs, include `stackState` in normalized `pr-closeout/v1` input. `stable` or `not_applicable` may continue; `unstable` and required `unknown` state must remain branch blockers until parent, lower-layer, and base evidence is current. This is read-only evidence and does not infer merge readiness.
...
- For this repository, keep `## Validation` structured with `Regression coverage`, one `Command: <exact command> -> pass|fail|blocked` line per command, and `Untested or blocked paths`. Bind each command to its outcome so reviewers do not have to reconcile parallel lists.
