...
from acceptance closure. Use `Linear reference` for the issue link itself, and
use `Linked issue relationship` to state whether the PR implements/closes the
linked acceptance criteria, is preparatory/enabling work, or is standalone. If a
PR references a parent goal without completing its acceptance IDs, say so
