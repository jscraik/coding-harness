...
The PR template and `pr-template-gate` reject repeated-steering admissions that do not name both a durable meta-behavior proof and a learning or reinforcement destination. They reject line-level or design-pattern correction admissions that do not include a pattern scope inventory with the inferred principle, sibling search, siblings changed, and siblings intentionally unchanged or deferred with reasons. They also reject repeated-error admissions that do not record web/upstream research, 3-5 candidate fixes, the chosen efficient fix, and what was implemented. `n.a.` is valid only when the PR body does not admit steering feedback, repeated user correction, pattern-bearing line feedback, or repeated troubleshooting failure, or when it names a tracked exception.
