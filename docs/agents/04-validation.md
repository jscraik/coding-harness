...
- Run `pnpm run quality:behavior-tests` when high-trust evidence-bearing tests change; the guard keeps suites registered in `src/lib/testing/behavior-test-suites.json` covered by `expectBehavior({ given, should, actual, expected })`.
- Run `pnpm run quality:git-env-sanitizer` when git child-process environment handling changes; the guard keeps manual cleanup routed through `src/lib/git/safe-env.ts`.
