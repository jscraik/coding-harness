...
- `scripts/check-environment.sh` must normalize the local execution path before tool discovery by adding standard mise, Homebrew, and local binary directories ahead of existing PATH entries when present. On macOS it should re-exec `/opt/homebrew/bin/bash` when the system Bash is older than the script contract requires.
