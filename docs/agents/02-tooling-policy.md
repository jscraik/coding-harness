...
`scripts/codex-enforced` should treat `main` as protected task-entry context: auto-create a dedicated `codex/<slug>` worktree via `scripts/new-task.sh --bootstrap`, then re-launch Codex inside the new worktree.
