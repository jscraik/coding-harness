...
Generated environment setup scripts must initialize `PATH` before adding known tool locations so `set -u` bootstrap shells are safe on minimal CI agents and fresh local worktrees.
