...
| PR queue admission             | `pnpm run pr:queue -- --json --require-ready [--require-review-artifact]`                           | Join required checks, merge state, paginated GraphQL review threads, and optional substantive provider-review evidence for every open PR before mutation; the opt-in artifact flag keeps the default solo-maintainer path permissive while distinguishing observed, rate-limited, action-only, and missing provider signals |
