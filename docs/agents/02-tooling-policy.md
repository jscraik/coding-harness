...
- `scripts/check-semgrep-changed.sh` pins the Semgrep version (`semgrep==1.153.1`) for changed-file hooks, and `scripts/check-semgrep-full.sh` reuses the same pinned runtime for full-repository CI scans. The CircleCI `security-scan` workflow calls `bash scripts/check-semgrep-full.sh` for the repo-owned Semgrep lane and runs `snyk-dependency-scan` through the pinned `snyk/snyk@2.3.0` orb with `monitor-on-build: false` and `fail-on-issues: false`. This keeps CircleCI as the repo-run Snyk reporting lane while the external GitHub Snyk PR check remains the blocking dependency-delta signal. Keep both Semgrep scripts, the `pnpm semgrep:changed` script, the Snyk orb version, and the CircleCI `security-scan` workflow aligned when changing repo-run security scanning.
