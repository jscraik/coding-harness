...
uv-backed Python helper commands should run through `bash scripts/run-uv-python.sh <command> [args...]`. The wrapper owns the repository-scoped `UV_CACHE_DIR` and `UV_PROJECT_ENVIRONMENT` defaults and forces `UV_MALWARE_CHECK=1`, so package scripts, hook governance helpers, and generated scaffolds do not duplicate uv setup or bypass OSV malware screening for locked remote-registry installs. Keep the `uv` pin at `0.11.16` or newer so the active binary supports this preview control.
