...
| PR queue admission             | `pnpm run pr:queue -- --json --require-ready`                                                       | Join current checks, merge state, and GraphQL review-thread state for every open PR before any new mutation; exits non-zero while the queue is blocked                                               |
