...
For example, a detached checkout in `coding-harness` at `abcdef1` should attach to `jscraik/feature/coding-harness-worktree-abcdef1`, adding a numeric suffix only if that branch already exists. Tooling that reads branch names must treat this as an agent worktree branch, not as a Linear-tracked `codex/<issue-key>-<slug>` task branch.
...
`scripts/new-task.sh` should fetch the latest remote base branch (`origin/<base>`) and create the worktree branch from that updated ref under `jscraik/feature/<issue-key>-<slug>` by default so fresh task worktrees start from current upstream state while matching the downstream scaffold branch-prefix contract.
