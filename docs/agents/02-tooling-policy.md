...
| Tests (CircleCI fast lane) | `pnpm test:ci:fast` | Runs the non-pilot suite plus the isolated `ci-migrate` lane used by `.circleci/config.yml` |
...
1. `pnpm test:ci`
2. `pnpm test:ci:fast` when you specifically need parity with the PR fast lane defined in `.circleci/config.yml`
