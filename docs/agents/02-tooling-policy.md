...
- `scripts/check-environment.sh` must fail generated `prek` hook drift when installed `pre-commit`, `pre-push`, or `commit-msg` shims lack the worktree-local `PREK_HOME` patch. Repair drift with `node scripts/setup-git-hooks.js` or `make hooks`, not raw `prek install`.
