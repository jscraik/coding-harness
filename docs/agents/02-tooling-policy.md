...
`scripts/prepare-worktree.sh` is the lightweight bootstrap lane for new worktrees; it trusts the active worktree's repo-local `.mise.toml` when `mise` is available, then ensures dependencies are installed in the active worktree so pre-push hooks that execute `pnpm` gates do not fail from missing `node_modules/`.
