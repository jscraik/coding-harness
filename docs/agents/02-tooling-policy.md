...
uv-backed Python helper commands should run through `bash scripts/run-uv-python.sh <command> [args...]`. The wrapper unconditionally owns worktree-local `UV_CACHE_DIR` and `UV_PROJECT_ENVIRONMENT` paths and forces `UV_MALWARE_CHECK=1`, so inherited user-global caches cannot make package scripts, hook helpers, or generated scaffolds depend on another checkout's mutable state or bypass OSV malware screening for locked remote-registry installs. Keep the `uv` pin at `0.11.16` or newer so the active binary supports this preview control.
...
- `scripts/run-prek.sh` is the supported direct `prek` entrypoint because it unconditionally uses the active worktree's `.cache/prek` directory before invoking `prek`. Use it for config validation and manual hook operations instead of raw `prek` commands in sandboxed runs.
...
| PR queue admission             | `pnpm run pr:queue -- --json --require-ready`                                                       | Join required checks, merge state, paginated GraphQL review threads, and optional substantive provider-review evidence for every open PR before mutation; the opt-in artifact flag keeps the default solo-maintainer path permissive while distinguishing observed, rate-limited, action-only, and missing provider signals |
| Existing-repo upgrade matrix   | `pnpm test:harness-upgrade-matrix -- <repo>... --json`                                               | Run the built CLI package `init --update --dry-run --json` path across existing repos and fail if any target git status changes or omits update-mode evidence; emit the matrix as JSON for downstream `fleet-plan` consumption                                                                                                                                                    |
...
| Surface | Owner | Successful output | Failure / stop behavior |
| --- | --- | --- | --- |
| CodeRabbit learnings import, gate, and promote | `learnings` command | A parsed learning report or promotion-candidate JSON bound to the requested source and files | Stop on an unreadable source, invalid provider rows, or an unsafe promotion; do not rewrite the source export |
| Review context pack | `review-context` command | Review-context JSON listing applicable learnings and validation entries for the requested files | Stop when the source or file list is missing, invalid, or cannot be matched deterministically |
| Validation plan | `validation-plan` command | `validation-plan/v1` JSON with canonical commands and network-required lanes separated | Stop on missing `--files`, malformed learning evidence, or an unavailable command contract |
| North-star feedback | `north-star-feedback` command | A feedback report with learning-loop and promotion signals | Stop when the evidence source is unavailable or the report cannot distinguish observation from causal measurement |
| Artifact provenance gate | `artifact-gate` command | A zero-blocker provenance result for the declared files | Stop on missing provenance, source/template mismatch, or an undeclared generated artifact |
| CI ownership gate | `ci-ownership-gate` command | A pass result proving the primary PR workflow and independent checks have the declared owners | Stop on ownership drift or an unknown required check |
| PR closeout evidence | `pr-closeout` command | `pr-closeout/v1` evidence with separate local, hosted, review, and merge claims | Stop and report blockers when GitHub state, closeout gates, or claim inputs are unavailable |
| PR triage snapshot | `pr:triage` script | Read-only PR metadata, check, review, and comment snapshot | Stop on GitHub/API failure; do not mutate the PR or infer missing review state |
| PR queue admission | `pr:queue` script | JSON admission for every open PR after checks, mergeability, threads, and selected review evidence are joined | Stop on any failed/pending check, unresolved thread, non-mergeable state, or missing required provider evidence |
| Existing-repo upgrade matrix | `test:harness-upgrade-matrix` script | Matrix JSON proving each target dry-run and unchanged Git status | Stop on a changed target, missing update-mode evidence, or a failed fixture |
| Fleet remediation plan | `fleet-plan` command | Agent-native plan JSON with safe dry-run actions and explicit approvals | Stop when the matrix is missing, stale, malformed, or proposes an unsafe live mutation |
| Current-repo upgrade preview | `upgrade` command | Structured dry-run JSON containing `updateMode`, `trackedManifest`, and `updateDetails` | Stop on invalid contract state, missing manifest, or any non-dry-run mutation request |
...
- Generated `scripts/check-environment.sh` keeps its local, mise, and global
  runner order. When none is available, set
  `HARNESS_CLI_ALLOW_NPM_EXEC=1` to opt into the anonymous public `npm exec`
  fallback; without that opt-in, the legacy auth-bound diagnostic remains
  fail-closed.
...
- The `mise` command is available on PATH
  ...
  `scripts/codex-preflight.sh` is a CLI script and should be executed, not sourced. Use `bash scripts/codex-preflight.sh --stack auto --mode optional` for standard checks; routine mode does not invoke Local Memory. `--mode required` is an explicit Local Memory diagnostic or acceptance lane. Legacy positional invocations are compatibility-only and remain non-mutating by default. The stack/mode shorthand `bash scripts/codex-preflight.sh auto required` remains supported for older agents and retains required-mode semantics.
  ...
  `scripts/verify-work.sh` is the canonical repo-local verification entrypoint for harness-managed repos. Keep it repo-local, default it to non-mutating Local Memory handling, and scope its preflight path/binary expectations to scaffolded repo artifacts rather than codex-maintenance-only paths.
