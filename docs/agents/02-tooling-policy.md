...
Generated setup and tooling actions should prepend standard user/Homebrew/system tool paths before invoking `mise` so Codex-launched shells can find repo-required tools without relying on an interactive shell profile.
Generated script actions should include all package test/eval entrypoints that the harness exposes as first-class validation lanes, including `test:deep`, `test:evals`, `test:harness-upgrade-matrix`, `test:integration`, and `test:e2e` when those scripts exist.
