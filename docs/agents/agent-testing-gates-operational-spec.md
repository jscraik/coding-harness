...
| `S4 AUDIT`                           | `gate_pass`  | `pnpm run audit` passes (or no high/critical) | proceed to next gate                       | `S5 CHECK`     |
...
function execute(gateRun: GateRun, event: E): Transition {
	...
	switch (currentState) {
		...
		case S4_AUDIT:
			return runGate(event, "pnpm run audit", S5_CHECK);
