...
`scripts/check-environment.sh` may normalize PATH with standard local, mise, and Homebrew directories before checks, but it must keep that behavior deterministic and must only re-exec a known Homebrew Bash path when the system Bash cannot satisfy the repo script contract.
