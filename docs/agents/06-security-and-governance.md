...
- CircleCI now owns repo-run non-release security scanning in this repository. Keep `security-scan` in `.circleci/config.yml` and avoid reintroducing non-release GitHub Actions security workflows. The workflow includes the repo-run Semgrep lane and a Snyk dependency lane through the pinned `snyk/snyk@2.3.0` orb; keep `monitor-on-build: false` unless external Snyk project snapshot writes are explicitly approved, and keep `fail-on-issues: false` so historical whole-repo vulnerability findings report without blocking PRs that the external GitHub Snyk delta check has cleared. Semgrep Cloud is enforced separately through the external GitHub App check `semgrep-cloud-platform/scan`; do not fold that required check into CircleCI workflow metadata.
...
`scripts/check-semgrep-changed.sh` is intentionally narrow: it compares `HEAD` to the upstream merge-base (or the nearest main/master fallback), filters to changed implementation files under `src/**`, and runs only the local `scripts/semgrep-pre-push.yml` ruleset. That keeps the local lane useful without duplicating the full CI security scan. Keep the script pinned to the same Semgrep version used by the CircleCI `security-scan` lane in `.circleci/config.yml` (`semgrep==1.153.1`) so local and CI findings stay aligned. The same workflow also runs Snyk through the pinned CircleCI orb with `SNYK_TOKEN` supplied by CircleCI project environment variables, monitoring disabled by default, and issue findings reported without failing the workflow; the external GitHub Snyk PR check remains the blocking dependency-delta signal. Keep Semgrep Cloud's `semgrep-cloud-platform/scan` branch-protection check as an independent external app gate.
