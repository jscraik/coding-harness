...
- For preflight template maintenance, author downstream-facing baseline changes in `src/templates/codex-preflight.sh`. Project runtime `scripts/codex-preflight.sh` is allowed to diverge intentionally; run `node scripts/sync-codex-preflight.cjs --write` only when an explicit resync is required.
