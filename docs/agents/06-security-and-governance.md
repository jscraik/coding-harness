...
- Project-local Codex roles under `.codex/agents/<role>/<role>.toml` are
  repository-scoped execution policy. Keep reviewer roles read-only, keep
  `harness-toolsmith` as the only workspace-write capability-building role,
  validate the inventory and harness-themed role nicknames with
  `pnpm codex:agents:guard`, and keep these roles out of global Codex config
