...
- `.mise.toml` trust behavior: `scripts/prepare-worktree.sh` automatically trusts the repository's `.mise.toml` via `mise trust --yes` when both the file and `mise` CLI are present. This grants mise permission to execute the tool definitions in `.mise.toml` without interactive prompts. Security implication: The trust operation runs without user confirmation when conditions are met, relying on repository checkout integrity and the user's PATH configuration. The trust step is skipped silently if `.mise.toml` is absent or `mise` is unavailable, and failures print a warning but do not block worktree preparation.
