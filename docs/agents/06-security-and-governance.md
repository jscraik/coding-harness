...
- Keep `scripts/codex-preflight.sh` executable as a CLI script and invoke it with `bash scripts/codex-preflight.sh --stack auto --mode optional` for routine work. `--mode required` remains an explicit Local Memory diagnostic or acceptance lane; do not source the script. Legacy positional invocations are compatibility-only and default to optional diagnostics.
- Treat `scripts/verify-work.sh` as the canonical repo-local verification entrypoint; it should keep optional Local Memory diagnostics and repo-scoped preflight expectations without depending on codex-maintenance-only paths.
