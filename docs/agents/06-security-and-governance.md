...
- Harness-managed consumer repositories are a defined exception: `scripts/check-environment.sh` should prefer a repo-local CLI runner or wrapper, then a mise-resolved harness binary (`mise which harness`), and use a global npm install of `@brainwav/coding-harness` only as the final fallback. Public package installs do not require registry credentials; reserve `NPM_TOKEN` wiring for private dependencies, private registries, or explicitly selected token-mode publishing.
...
- uv-backed Python validation helpers must route through
  `scripts/run-uv-python.sh` so its non-inherited worktree-scoped uv cache and
  virtual environment settings and `UV_MALWARE_CHECK=1` stay centralized and
  do not drift across package scripts, hook governance helpers, or generated init
...
`bash scripts/check-codestyle-parity.sh` is a required verification surface in the bootstrap lane (`bash scripts/codex-preflight.sh --stack auto --mode optional`) and in broader readiness checks (`bash scripts/verify-work.sh`).
...
- Keep `scripts/codex-preflight.sh` executable as a CLI script and invoke it with `bash scripts/codex-preflight.sh --stack auto --mode optional` for routine work. Optional mode does not invoke Local Memory. `--mode required` remains an explicit Local Memory diagnostic or acceptance lane; do not source the script. Legacy positional invocations are compatibility-only and non-mutating by default.
- Treat `scripts/verify-work.sh` as the canonical repo-local verification entrypoint; it should keep Local Memory outside routine execution and retain repo-scoped preflight expectations without depending on codex-maintenance-only paths.
...
`scripts/run-prek.sh` is the supported direct `prek` entrypoint because it unconditionally uses the active worktree's `.cache/prek` directory before invoking `prek`. `scripts/setup-git-hooks.js` must run `scripts/run-prek.sh install --overwrite`, resolve the installed hook directory with `git rev-parse --git-path hooks`, and patch generated `prek` shims to derive `WORKTREE_ROOT` with `git rev-parse --show-toplevel` before setting `PREK_HOME="$WORKTREE_ROOT/.cache/prek"` so inherited cache locations cannot cross worktree boundaries, hook logs/cache writes stay local under sandboxed executions, and linked worktrees do not write through the shared git directory.
