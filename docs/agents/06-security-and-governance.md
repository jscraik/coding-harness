...
- For preflight template maintenance, author downstream-facing changes in `src/templates/codex-preflight.sh` and sync the runtime mirror with `node scripts/sync-codex-preflight.cjs --write`; `pnpm lint` enforces parity with the matching `--check` gate.
