...
- Generated Codex environment setup may prepend known user/Homebrew/system tool directories to `PATH` and may trust `.mise.toml` in the active worktree, but it must not perform unbounded global installs or silently bypass the repository readiness gates.
